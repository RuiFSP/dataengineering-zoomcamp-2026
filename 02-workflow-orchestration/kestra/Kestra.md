...
**Note:** If you plan to use Kestra's AI features, ensure you set up the `GEMINI_API_KEY` environment variable. You can do this by creating a `.env` file in the same directory as your `docker-compose.yml` and adding the following line:

```env
GEMINI_API_KEY=your_actual_api_key_here
```

Additionally, update your `docker-compose.yml` file to include the following configuration under the `kestra` service:
...
**Reference:** [Google Credentials in Kestra](https://kestra.io/docs/how-to-guides/google-credentials)

#### Step 1: Create a GCP Service Account

1. Go to **GCP Console** → **IAM & Admin** → **Service Accounts**
2. Create a new service account (e.g., `rui-zoomcamp`)
3. Grant it the **Owner** role (or at minimum: **Storage Admin** + **BigQuery Admin**)
4. Create and download the JSON key file

#### Step 2: Configure GCP Credentials in Kestra

Instead of storing credentials in the KV store, we use Kestra's secrets management for better security:

1. **Add the JSON to `.env` file:**
   ```env
   GEMINI_API_KEY=your_gemini_api_key
   GCP_SERVICE_ACCOUNT_JSON={"type":"service_account","project_id":"your-project",...}
   ```
   Note: The JSON should be on a single line

2. **Ensure docker-compose.yml references `.env_encoded`:**
   
   The `kestra` service should have:
   ```yaml
   env_file:
     - .env_encoded
   ```
   This tells Docker Compose to load the encoded secrets file.

3. **Encode the secrets to `.env_encoded`:**
   ```bash
   cd 02-workflow-orchestration/kestra
   echo "SECRET_GEMINI_API_KEY=$(echo -n 'your_gemini_key' | base64)" > .env_encoded
   grep "^GCP_SERVICE_ACCOUNT_JSON=" .env | sed 's/^GCP_SERVICE_ACCOUNT_JSON=//' | base64 -w 0 | sed 's/^/SECRET_GCP_SERVICE_ACCOUNT_JSON=/' >> .env_encoded
   ```
   
   This creates `.env_encoded` with base64-encoded values prefixed with `SECRET_`.
...
4. **Restart Docker Compose:**
   ...
   docker compose up -d
   ```
   This loads the new secrets into Kestra.

4. **Configure Flow 06 with your GCP settings:**
   
   Adjust the following flow [`06_gcp_kv.yaml`](flows/06_gcp_kv.yaml) to include your GCP project ID, BigQuery dataset and GCS bucket name (_along with their location_) as KV Store values:
   - GCP_PROJECT_ID (must match your service account's project)
   - GCP_LOCATION
   - GCP_BUCKET_NAME
   - GCP_DATASET

5. **Update Flow 07 to use secrets:**
   
   The flow [`07_gcp_setup.yaml`](flows/07_gcp_setup.yaml) uses `{{ secret('GCP_SERVICE_ACCOUNT_JSON') }}` instead of storing credentials in KV:
   ```yaml
   pluginDefaults:
...
**Why use secrets instead of KV store?**
- Secrets are base64-encoded and not visible in the UI
- Better security practice for sensitive credentials
- Secrets are injected at runtime, not stored in the database
...
Run the flows in order:
1. Run `06_gcp_kv.yaml` to store configuration in KV
2. Run `07_gcp_setup.yaml` to create GCS bucket and BigQuery dataset
...
### 2.4.3 Cleanup GCP Resources

To avoid unexpected charges after completing the module, clean up your GCP resources:

**Option 1: Using GCP Console (Recommended)**

1. **Delete GCS Bucket:**
   - Go to **Cloud Storage** → **Buckets**
   - Select your bucket (e.g., `kestra-zoomcamp-rui-demo`)
   - Click **Delete** → Confirm deletion
   - This removes the bucket and all files inside

2. **Delete BigQuery Dataset:**
   - Go to **BigQuery** → Select your project
   - Find your dataset (e.g., `zoomcamp`)
   - Click the three dots → **Delete dataset**
   - Check "Delete all tables in the dataset"
   - Confirm deletion

3. **Optional - Delete Service Account:**
   - Go to **IAM & Admin** → **Service Accounts**
   - Select your service account (e.g., `rui-zoomcamp`)
   - Click **Delete** → Confirm
   - Note: Only do this if you won't use it again

**Option 2: Using gcloud CLI**

```bash
# Delete GCS bucket and contents
gcloud storage rm --recursive gs://kestra-zoomcamp-rui-demo

# Delete BigQuery dataset and all tables
bq rm -r -f -d kestra-sandbook:zoomcamp

# Optional: Delete service account
gcloud iam service-accounts delete rui-zoomcamp@kestra-sandbook.iam.gserviceaccount.com
```

**What incurs costs:**
- ✅ **GCS storage:** Charged per GB per month (even small amounts add up)
- ✅ **BigQuery storage:** Charged per GB per month for stored tables
- ✅ **BigQuery queries:** Charged per TB of data processed
- ❌ **Service accounts:** Free (no cost to keep them)
- ❌ **Empty datasets/buckets:** Free once data is deleted

**Best practice:** Delete resources immediately after completing the module exercises, or at least delete the data inside buckets/datasets. You can recreate them anytime using Flow 07.
